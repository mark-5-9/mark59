 ...
 *
 * <p>The location of the spreadsheet containing the server profile details is generally expected to be set via
 * the property 'mark59.server.profiles.excel.file.path' (in mark59.properties). A path override is also available
 * on the Java Request parameter list.
 *
 * <p>Using details provided on the spreadsheet, calls a function which controls and executes commands on the target servers,
	...
	public SampleResult runTest(JavaSamplerContext context) {
		...
		if (StringUtils.isAllBlank(excelFilePath)) {
			...
			try {
				...
				LOG.error("excel spreadsheet " + excelFilePath + " file error. Msg : " + e.getMessage());
				System.out.println("excel spreadsheet " + excelFilePath + " file error. Msg : " + e.getMessage());
				e.printStackTrace();
			}
