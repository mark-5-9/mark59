...
public class ServerMetricsCaptureUtils {
	...
	public static void logUnexpectedException(WebServerMetricsResponsePojo response, Throwable e, String source) {
		...
		if (response != null) {
			String erroredServerProfileMsg = "        occurred using server profile :" + response.getServerProfileName();
			LOG.error(erroredServerProfileMsg);
			System.out.println(erroredServerProfileMsg);
		}
	}
