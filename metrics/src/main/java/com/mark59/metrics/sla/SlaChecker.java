...
public class SlaChecker {
	...
	public List<SlaTransactionResult> listTransactionsWithFailedSlas(String graphApplication, Collection<Transaction> transactions, SlaDAO slaDAO) {
		
		List<SlaTransactionResult> transactionsWithFailedSlas = new ArrayList<SlaTransactionResult>();

		String defaultSlaTxnId 	 = SlaUtilities.deriveDefaultSLAtransactionId(graphApplication);		
	
		for (Transaction transaction : transactions) {
			Sla transactionSla = slaDAO.getSla(graphApplication, transaction.getTxnId(), defaultSlaTxnId);
			SlaTransactionResult slaTransactionResult = this.checkIfTransactionMeetDatabaseSLA(defaultSlaTxnId, transaction, transactionSla);	
	...
	public List<String> checkForMissingTransactionsWithDatabaseSLAs(String graphApplication, String runTime, SlaDAO slaDAO) {
		
		List<String> slasWithMissingTxns = slaDAO.getSlasWithMissingTxnsInThisRun(graphApplication, runTime  );		
	...
	public SlaTransactionResult checkIfTransactionMeetDatabaseSLA(String slaApplicationKey, Transaction transaction, Sla transactionSla) {
