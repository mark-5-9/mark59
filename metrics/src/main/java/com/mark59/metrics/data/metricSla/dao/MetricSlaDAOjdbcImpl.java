...
public class MetricSlaDAOjdbcImpl implements MetricSlaDAO {
	...
	public void insertData(MetricSla metricSla) {
		...
		jdbcTemplate.update(sql,
				new Object[] { metricSla.getApplication(),metricSla.getMetricName(),metricSla.getMetricTxnType(),
								metricSla.getValueDerivation(), metricSla.getSlaMin(), metricSla.getSlaMax(), metricSla.getComment() });
	}
...
//			System.out.println("populating application in dropdown list : " + row.get("APPLICATION")  ) ;
	...
	@Override
	public List<String> getSlasWithMissingTxnsInThisRun(String application, String cIapplication, String runTime) {
...
//					+ "     where APPLICATION = '" + cIapplication + "' "
...
////			System.out.println("populating application in dropdown list : " + row.get("APPLICATION")  ) ;
//		}
		missingTransactions.add( "** MISSING SLA METRICS NOT YET IMPLEMENTED **" );
