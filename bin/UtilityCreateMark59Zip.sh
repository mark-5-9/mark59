...
#   | 
#   |  Source is also included for mark59-scripting-samples and mark59-scripting-sample-dsl projects (the  . 
#   |  target folder is excluded for mark59-scripting-sample-dsl projects - so it contains source only)     
#   |  
...
rsync -a -m --exclude '.*' --exclude '*.yml' --exclude '*/classes' --exclude '*/test-classes' --exclude '*/maven*' --exclude 'archive-tmp' --exclude '*/m2e-wtp*' --exclude '*/surefire*' --exclude '*.log' --exclude '*.original' --exclude '/pom.xml' --exclude 'bin' --exclude 'databaseScripts'  --exclude 'mark59-scripting-sample-dsl/target' --exclude 'mark59-core' --exclude 'mark59-datahunter' --exclude 'mark59-datahunter-api' --exclude 'mark59-metrics' --exclude 'mark59-metrics-api' --exclude 'mark59-metrics-common' --exclude 'mark59-results-splitter' --exclude 'mark59-selenium-implementation' --exclude 'mark59-trends' --exclude 'mark59-trends-load' "${SOURCE_DIR}" "${DEST_DIR}"
