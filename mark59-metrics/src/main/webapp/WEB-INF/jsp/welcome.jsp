...
<div><img style="max-width: 100%;" src="images/mark59_banner_thin.png" alt="Mark59 Banner"/></div>
...
The Metrics web application contains three main data structures: Server Profiles, Commands, and Response Parsers.</p>
...
Depending on the Commands used, they may need parameter values to be set.</p>
...
<p>There are also some pre-defined parameters available (listed above the command panel).</p>
...
A Groovy command does not use a response parser, as you should be able to extract the metric(s) you need using the Groovy command itself.</p>

<p><b>Notes on Key Connection Parameters..</b></p></p>
...
The Java library Jsch used for Nix connectivity has a limitation with SSH private/public key formats,
 so you should use the 'classic OpenSSH' format.</p>

<p>To convert an already existing key:</p></p>
<pre>ssh-keygen -p -f privateKeyFile -m PEM -P passphrase -N passphrase</pre>

<p>For a key without a passphrase, use a blank string. Eg:</p>
<pre>ssh-keygen -p -f ~/.ssh/myfavsshprvkey -m PEM -P "" -N ""</pre>

<p>When creating a new key, just add the <span class="c10n">-m PEM</span> option to the ssh-keygen command. Eg:</p>
<pre>ssh-keygen -m PEM -t rsa -f ~/.ssh/mynewfavsshkey</pre>

<p>In Mark59, the pre-defined parameter holding the location of the private key is SSH_IDENTITY.</p>
...
<p>For a <b>Windows PowerShell connection</b> using a Credential object built using a user-defined Key,
 ...
 To create the necessary configuration elements, you run PowerShell locally. Eg:</p>
...
<p>In this example the parameter values would be:</p>
...
<p>Also put your userid in the Username field (but do not enter a password).</p>

<p>This is the preferred approach as:</p>
<ul>
<li>you never need to enter the user's password into Mark59, and</li>
<li>if you do not use a user-defined key, the system generates an internal key base on the computer and userid that was used.
 ...
 (pretty limiting for both the web application, and any use of a downloaded Server Profile spreadsheet).</li>
</ul>
